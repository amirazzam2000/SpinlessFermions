...
for epoch in range(start, epochs+1):
    ...
    with torch.no_grad():
        ...
        mean_elocal = torch.mean(elocal * ratio_no_mean) / r_mean
    ...
    loss2 = (ratio_no_mean * elocal).detach() * torch.mean(ratio_no_mean.detach() * logabs)/ r_mean.detach()
    ...
    loss = torch.mean(loss1) / r_mean.detach() - \
        torch.mean(loss2) / r_mean.detach()
    # loss = torch.mean(loss1) / r_mean 
